...
export const compact = R.pickBy(R.identity);

export const interchangeProps = R.curry((ind1, ind2, props, list) => {
  let vals1 = R.pick(props, list[ind1]);
  let vals2 = R.pick(props, list[ind2]);
  return R.pipe(
    R.update(ind1, R.merge(list[ind1], vals2)),
    R.update(ind2, R.merge(list[ind2], vals1))
  )(list);
});

export const setOrdre = R.curry((cond, fld, liste) => {
  let ordre = 0;
  return R.map(obj => R.ifElse(cond, R.assoc(fld, ordre++), R.identity)(obj))(liste);
});